...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" dir="{{ app()->getLocale() === 'ar' ? 'rtl' : 'ltr' }}">
    <head>
        ...
        <link href="https://fonts.googleapis.com/css2?family=Cairo:wght@200..1000&family=Inter:wght@300..700&display=swap" rel="stylesheet">
    ...
    <body class="bg-white text-gray-900" style="font-family: {{ app()->getLocale() === 'ar' ? "'Cairo'" : "'Inter'" }}, sans-serif;">
        <!-- Navigation Bar -->
                ...
                <div class="flex justify-between items-center h-16">
                    <a href="{{ route('home') }}" class="flex items-center hover:opacity-80 transition">
                        ...
                        <div class="flex gap-3">
                                ...
                                <a href="{{ route('home') }}" class="px-6 py-2 bg-gradient-to-r from-[#dd208e] to-[#b01670] text-white rounded-lg hover:shadow-lg transition text-sm">{{ __('admin.home') }}</a>
            ...
            <section class="px-4 sm:px-6 lg:px-8 bg-gradient-to-b from-[#fde4f1] to-white py-12 sm:py-16">
                <div class="max-w-4xl mx-auto">
                    ...
                    <div class="flex flex-wrap gap-4 items-center text-sm text-gray-600 mb-8">
                        ...
                        <span>•</span>
                        <span>{{ __('admin.last_updated') }} {{ $blog->updated_at->format('d F Y H:i') }}</span>
                    </div>
                </div>
            </section>
            ...
            <article class="px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
                <div class="max-w-4xl mx-auto">
                    ...
                    <div class="bg-gradient-to-r from-[#fde4f1] to-[#fff5f9] rounded-lg p-6 sm:p-8 mb-10 border border-[#f0b3d8]">
                        <h2 class="text-lg sm:text-xl font-bold text-gray-900 mb-3">{{ __('admin.summary') }}</h2>
                    ...
                    <div class="mt-16 pt-8 border-t-2 border-gray-200">
                        <h3 class="text-2xl font-bold text-gray-900 mb-6">{{ __('admin.more_articles') }}</h3>
                        @php
                                ...
                                @foreach($relatedBlogs as $relatedBlog)
                                    <a href="{{ route('blogs.public.show', $relatedBlog->id) }}" class="group bg-white rounded-lg shadow-md hover:shadow-lg transition overflow-hidden border border-gray-100">
                                        ...
                                        <div class="p-4">
                                            ...
                                            <span class="text-[#dd208e] font-medium text-sm">{{ __('admin.read_more') }} ←</span>
                                        </div>
                                    </a>
                                @endforeach
                            </div>
                        @else
                            <p class="text-gray-600">{{ __('admin.no_related_articles') }}</p>
                        @endif
                    </div>

                    <!-- CTA Section -->
                    <div class="mt-16 p-8 sm:p-12 bg-gradient-to-r from-[#dd208e] to-[#b01670] rounded-lg text-white text-center">
                        <h3 class="text-2xl sm:text-3xl font-bold mb-4">{{ __('admin.improve_salon_cta') }}</h3>
                        <p class="text-lg text-red-100 mb-6">{{ __('admin.start_journey_cta') }}</p>
                        <a href="{{ route('register') }}" class="inline-block px-8 py-3 bg-white text-[#dd208e] font-bold rounded-lg hover:shadow-lg transition">
                            {{ __('admin.free_registration') }}
                        </a>
                    </div>
                </div>
            </article>
            ...
            <div class="max-w-6xl mx-auto">
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 sm:gap-8 mb-8">
                    <div class="space-y-3 sm:space-y-4">
                        ...
                        <p class="text-xs sm:text-sm">{{ __('admin.footer_description') }}</p>
                    </div>
                    <div>
                        <h4 class="font-bold text-gray-900 mb-3 sm:mb-4 text-base sm:text-lg">{{ __('admin.product') }}</h4>
                        <ul class="space-y-1.5 sm:space-y-2 text-xs sm:text-sm">
                            <li><a href="{{ route('home') }}#features" class="hover:text-gray-700 transition">{{ __('admin.features') }}</a></li>
                            <li><a href="{{ route('home') }}#pricing" class="hover:text-gray-700 transition">{{ __('admin.pricing') }}</a></li>
                            
                        </ul>
                    </div>
                    <div>
                        <h4 class="font-bold text-gray-900 mb-3 sm:mb-4 text-base sm:text-lg">{{ __('admin.company') }}</h4>
                        <ul class="space-y-1.5 sm:space-y-2 text-xs sm:text-sm">
                            <li><a href="{{ route('home') }}#about" class="hover:text-gray-700 transition">{{ __('admin.about_us') }}</a></li>
                            <li><a href="{{ route('blogs.public.index') }}" class="hover:text-gray-700 transition">{{ __('admin.blog') }}</a></li>
                            
                        </ul>
                    </div>
                    <div>
                        <h4 class="font-bold text-gray-900 mb-3 sm:mb-4 text-base sm:text-lg">{{ __('admin.legal') }}</h4>
                        <ul class="space-y-1.5 sm:space-y-2 text-xs sm:text-sm">
                            
                            <li><a href="{{ route('policy') }}" class="hover:text-gray-700 transition">{{ __('admin.terms_and_conditions') }}</a></li>
                            <li><a href="{{ route('home') }}#contact" class="hover:text-gray-700 transition">{{ __('admin.contact') }}</a></li>
                        </ul>
                    </div>
                </div>
                
                <div class="border-t border-gray-800 pt-6 sm:pt-8 text-center text-xs sm:text-sm">
                    <p>&copy; 2026 . {{ __('admin.all_rights_reserved') }}</p>
                    <a href="https://www.instagram.com/mohamed_izeldeen/" target="_blank" rel="noopener noreferrer" class="inline-flex items-center justify-center gap-2 text-[#dd208e] hover:text-[#b01670] mt-2">
                        ...
                        <span>{{ __('admin.developed_by') }}</span>
                    </a>
                </div>
            </div>
        </footer> 
