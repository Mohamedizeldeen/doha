...
@section('page-title', __('admin.new_booking'))

@section('content')
<div class="mb-6">
    <h2 class="text-2xl font-bold text-gray-800">{{ __('admin.new_booking') }}</h2>
</div>

<div class="bg-white rounded-lg shadow-md p-8 max-w-2xl">
    <form method="POST" action="{{ route('booking.store', $salon) }}" class="space-y-6" id="bookingForm">
        @csrf
        ...
        <div>
            ...
            <select id="service_id" name="service_id" required
                ...
                @foreach($services as $service)
                    <option value="{{ $service->id }}" data-duration="{{ $service->duration_minutes }}" @selected(old('service_id') == $service->id)>
        ...
        <!-- Staff Selection (dynamic based on availability) -->
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">{{ __('admin.staff_member') }}</label>
            <div id="staff-availability-hint" class="text-sm text-amber-600 mb-2">
        ...
        <div class="flex gap-4 pt-6">
            <button type="submit" id="submitBtn" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-6 rounded-lg transition disabled:opacity-50 disabled:cursor-not-allowed" disabled>
                {{ __('admin.save') }}
            </button>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function fetchAvailableStaff() {
        ...
        fetch(`/api/salon/${salonId}/available-staff?date=${date}&time=${time}&service_id=${serviceId}`)
            .then(r => r.json())
            .then(data => {
    ...
    function updateSubmitBtn() {
        submitBtn.disabled = !staffInput.value;
    }

    // Expose pickStaff globally
    ...
    // Initial check if values already exist (e.g. validation error redirect)
    if (serviceSelect.value && datetimeInput.value) {
        fetchAvailableStaff();
    }
});
</script>
