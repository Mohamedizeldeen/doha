...
@section('content')
<div class="mb-6">
    <h2 class="text-2xl font-bold text-gray-800">{{ __('admin.add_category') }}</h2>
</div>

<div class="bg-white rounded-lg shadow-md p-8 max-w-2xl">
    <form method="POST" action="{{ route('category.store', $salon) }}" class="space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" id="name_ar" name="name_ar" value="{{ old('name_ar') }}" required
                    ...
                    placeholder="{{ app()->getLocale() === 'ar' ? 'مثال: العناية بالشعر' : 'e.g. Hair Care' }}">
            ...
            <div>
                ...
                <input type="text" id="name_en" name="name_en" value="{{ old('name_en') }}" required
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-transparent"
                    placeholder="{{ app()->getLocale() === 'ar' ? 'مثال: Hair Care' : 'e.g. Hair Care' }}">
