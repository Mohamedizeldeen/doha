...
@section('page-title', __('admin.add_client'))

@section('content')
<div class="mb-6">
    <h2 class="text-2xl font-bold text-gray-800">{{ __('admin.add_new_client') }}</h2>
</div>
...
<div class="bg-white rounded-lg shadow-md p-8 max-w-2xl">
    <form method="POST" action="{{ route('client.store', $salon) }}" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                <label for="client_code" class="block text-sm font-medium text-gray-700 mb-2">{{ __('admin.client_code_optional') }}</label>
                <input type="text" id="client_code" name="client_code" value="{{ old('client_code') }}"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                <p class="text-xs text-gray-500 mt-1">{{ __('admin.auto_generate_code') }}</p>
