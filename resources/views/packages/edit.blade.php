...
<div style="margin-bottom: 1.5rem;">
    <h2 style="font-size: 1.15rem; font-weight: 700; color: #1f2937;">{{ __('admin.edit_package') }}</h2>
</div>

<form action="{{ route('package.update', [$salon, $package]) }}" method="POST">
    @csrf @method('PUT')
        ...
        <div style="display: grid; gap: 1rem;">
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.75rem;">
                <div>
                    ...
                    <input type="text" name="name_ar" value="{{ old('name_ar', $package->name_ar) }}" required style="width: 100%; padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 0.375rem; font-size: 0.85rem;">
                ...
                <div>
                    ...
                    <input type="text" name="name_en" value="{{ old('name_en', $package->name_en) }}" required style="width: 100%; padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 0.375rem; font-size: 0.85rem;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.75rem;">
                <div>
                    ...
                    <textarea name="description_ar" rows="2" style="width: 100%; padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 0.375rem; font-size: 0.85rem; resize: vertical;">{{ old('description_ar', $package->description_ar) }}</textarea>
                </div>
                <div>
                    <label style="font-size: 0.75rem; color: #6b7280; display: block; margin-bottom: 0.25rem;">{{ __('admin.description') }} (EN)</label>
                    <textarea name="description_en" rows="2" style="width: 100%; padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 0.375rem; font-size: 0.85rem; resize: vertical;">{{ old('description_en', $package->description_en) }}</textarea>
                </div>
            </div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.75rem;">
                <div>
                    ...
                    <input type="number" name="original_price" step="0.01" value="{{ old('original_price', $package->original_price) }}" required style="width: 100%; padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 0.375rem; font-size: 0.85rem;">
                </div>
                <div>
                    <label style="font-size: 0.75rem; color: #6b7280; display: block; margin-bottom: 0.25rem;">{{ __('admin.price') }} *</label>
                    <input type="number" name="price" step="0.01" value="{{ old('price', $package->price) }}" required style="width: 100%; padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 0.375rem; font-size: 0.85rem;">
                </div>
            </div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.75rem;">
                <div>
                    ...
                    <input type="date" name="valid_from" value="{{ old('valid_from', $package->valid_from?->format('Y-m-d')) }}" style="width: 100%; padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 0.375rem; font-size: 0.85rem;">
                </div>
                <div>
                    <label style="font-size: 0.75rem; color: #6b7280; display: block; margin-bottom: 0.25rem;">{{ __('admin.valid_until') }}</label>
                    <input type="date" name="valid_until" value="{{ old('valid_until', $package->valid_until?->format('Y-m-d')) }}" style="width: 100%; padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 0.375rem; font-size: 0.85rem;">
                </div>
            </div>
            ...
            <div>
                ...
                <div style="max-height: 200px; overflow-y: auto; border: 1px solid #d1d5db; border-radius: 0.375rem; padding: 0.5rem;">
                    @php $packageServiceIds = $package->services->pluck('id')->toArray(); @endphp
                    ...
                    <label style="display: flex; align-items: center; gap: 0.5rem; padding: 0.35rem 0; cursor: pointer; border-bottom: 1px solid #f3f4f6;">
                        <input type="checkbox" name="services[]" value="{{ $service->id }}" {{ in_array($service->id, old('services', $packageServiceIds)) ? 'checked' : '' }} style="accent-color: var(--primary);">
            ...
            <div style="display: flex; align-items: center; gap: 0.5rem;">
                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $package->is_active) ? 'checked' : '' }} style="accent-color: var(--primary);">
