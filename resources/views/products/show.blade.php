...
<div class="mb-6 flex justify-between items-center">
    <h2 class="text-2xl font-bold text-gray-800">{{ app()->getLocale() === 'ar' ? $product->name_ar : ($product->name_en ?? $product->name_ar) }}</h2>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    <div class="lg:col-span-2">
        <div class="bg-white rounded-lg shadow-md p-8">
            ...
            @if ($product->image)
                <div class="mb-6">
                    <img src="{{ asset('storage/' . $product->image) }}" alt="{{ app()->getLocale() === 'ar' ? $product->name_ar : ($product->name_en ?? $product->name_ar) }}" class="w-full max-h-96 object-cover rounded-lg">
                </div>
            @else
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                <div>
                    ...
                    <p class="text-xl font-bold text-gray-800">{{ app()->getLocale() === 'ar' ? $product->name_ar : ($product->name_en ?? $product->name_ar) }}</p>
                </div>
                ...
                <div>
                    <h3 class="text-sm font-medium text-gray-600 mb-2">{{ __('admin.price') }}</h3>
                    <p class="text-2xl font-bold text-green-600">{{ $product->price }} {{ $salon->currency }}</p>
                </div>

                <div>
                    <h3 class="text-sm font-medium text-gray-600 mb-2">{{ __('admin.stock') }}</h3>
                    <p class="text-2xl font-bold text-blue-600">{{ $product->stock_quantity }}</p>
                </div>

                <div>
                    <h3 class="text-sm font-medium text-gray-600 mb-2">{{ __('admin.status') }}</h3>
                ...
                <div>
                    <h3 class="text-sm font-medium text-gray-600 mb-2">{{ __('admin.date') }}</h3>
            ...
            @if ($product->description_ar || $product->description_en)
                <div class="border-t pt-6">
                    <h3 class="text-lg font-bold text-gray-800 mb-4">{{ __('admin.description') }}</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        @if ($product->description_ar)
                            <div>
                                <h4 class="font-medium text-gray-700 mb-2">{{ __('admin.description_ar') }}</h4>
                                <p class="text-gray-600">{{ app()->getLocale() === 'ar' ? $product->description_ar : ($product->description_en ?? $product->description_ar) }}</p>
                            </div>
                        @endif
                        @if ($product->description_en)
                            <div>
                                <h4 class="font-medium text-gray-700 mb-2">{{ __('admin.description_en') }}</h4>
    ...
    <div>
        <div class="bg-white rounded-lg shadow-md p-6">
            <h3 class="text-lg font-bold text-gray-800 mb-4">{{ __('admin.stock_management') }}</h3>
            
            <div class="mb-6 p-4 rounded-lg bg-blue-50">
                <p class="text-sm text-gray-600 mb-1">{{ __('admin.current_quantity') }}</p>
            ...
            <div class="space-y-2 text-sm">
                <div class="flex justify-between">
                    ...
                    <span class="font-bold {{ $product->isInStock() ? 'text-green-600' : 'text-red-600' }}">
                        {{ $product->isInStock() ? __('admin.in_stock') : __('admin.out_of_stock') }}
                    </span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-600">{{ __('admin.price_per_unit') }}:</span>
                    <span class="font-bold text-green-600">{{ $product->price }} {{ $salon->currency }}</span>
                </div>
                <div class="flex justify-between border-t pt-2 mt-2">
                    <span class="text-gray-600">{{ __('admin.total_value') }}:</span>
                    <span class="font-bold text-purple-600">{{ $product->price * $product->stock_quantity }} {{ $salon->currency }}</span>
                </div>
            </div>

            <div class="mt-6 pt-6 border-t">
                <p class="text-xs text-gray-500 text-center">{{ __('admin.edit_stock_hint') }}</p>
            </div>
        </div>
    </div>
</div>
