...
@section('page-title', __('admin.edit_article'))

@section('content')

<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-lg shadow-md p-8">
        <h1 class="text-3xl font-bold text-gray-900 mb-8">{{ __('admin.edit_article') }}</h1>
        ...
        <form action="{{ route('superAdmin.blogs.update', $blog->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="category" id="category" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" placeholder="{{ __('admin.category_example') }}" value="{{ old('category', $blog->category) }}">
                @error('category')
                    <span class="text-red-600 text-sm">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div>
                ...
                @if($blog->image)
                    <div class="mb-4">
                        ...
                        <p class="text-sm text-gray-600 mt-2">{{ __('admin.current_image') }}</p>
                    </div>
                @endif
                ...
                <p class="text-sm text-gray-600 mt-2">{{ __('admin.leave_empty_keep_image') }}</p>
                @error('image')
                    <span class="text-red-600 text-sm">{{ $message }}</span>
                @enderror
            </div>
