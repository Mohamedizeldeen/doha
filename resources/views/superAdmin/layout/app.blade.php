...
<head>
    ...
    <style>
        ...
        * { font-family: {{ app()->getLocale() === 'ar' ? "'Cairo'" : "'Inter'" }}, sans-serif; box-sizing: border-box; }
        ...
        .sidebar { position: fixed; top: 0; {{ app()->getLocale() === 'ar' ? 'right: 0;' : 'left: 0;' }} width: var(--sidebar-w); height: 100vh; background: linear-gradient(135deg, #0f172a 0%, #1e293b 50%, #0f172a 100%); z-index: 50; display: flex; flex-direction: column; transition: transform 0.3s cubic-bezier(0.4,0,0.2,1); overflow-y: auto; }
        ...
        .sidebar-brand .badge-sa { background: var(--primary); color: #fff; padding: 0.15rem 0.5rem; border-radius: 999px; font-size: 0.6rem; font-weight: 700; margin-{{ app()->getLocale() === 'ar' ? 'right' : 'left' }}: 0.25rem; }
        ...
        .nav-item .nav-badge { margin-{{ app()->getLocale() === 'ar' ? 'right' : 'left' }}: auto; background: rgba(239,68,68,0.2); color: #ef4444; padding: 0.1rem 0.5rem; border-radius: 999px; font-size: 0.7rem; font-weight: 600; }
        ...
        .sidebar-user-avatar { width: 36px; height: 36px; border-radius: 50%; background: linear-gradient(135deg, var(--primary), var(--primary-dark)); display: flex; align-items: center; justify-content: center; color: #fff; font-weight: 700; font-size: 0.85rem; flex-shrink: 0; }
        ...
        .topbar-btn:hover { border-color: var(--primary); color: var(--primary); background: #fdf2f8; }
        ...
        .lang-switch:hover { border-color: var(--primary); color: var(--primary); background: #fdf2f8; }
...
<body>
    <aside class="sidebar" id="sidebar">
        ...
        <nav class="sidebar-nav">
            ...
            <a href="{{ route('superAdmin.salons.index') }}" class="nav-item {{ request()->routeIs('superAdmin.salons.*') ? 'active' : '' }}">
                <i class="fas fa-store"></i><span>{{ __('admin.salons') }}</span>
            </a>
            <div class="nav-section-label">{{ __('admin.manage') }}</div>
            <a href="{{ route('superAdmin.bookings.index') }}" class="nav-item {{ request()->routeIs('superAdmin.bookings.*') ? 'active' : '' }}">
                <i class="fas fa-calendar-alt"></i><span>{{ __('admin.bookings') }}</span>
            </a>
            <a href="{{ route('superAdmin.products.index') }}" class="nav-item {{ request()->routeIs('superAdmin.products.*') ? 'active' : '' }}">
            ...
            <a href="{{ route('superAdmin.salesUsers.index') }}" class="nav-item {{ request()->routeIs('superAdmin.salesUsers.*') ? 'active' : '' }}">
                <i class="fas fa-user-tie"></i><span>{{ __('admin.sales_users') }}</span>
            </a>
        </nav>
