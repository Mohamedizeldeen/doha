...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-lg shadow-md p-8">
        <h1 class="text-3xl font-bold text-gray-900 mb-8">{{ __('admin.edit_product') }}</h1>
        ...
        <form action="{{ route('superAdmin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="bg-gray-50 p-4 rounded-lg">
                <p class="text-sm text-gray-600">{{ __('admin.salon') }}</p>
                <p class="text-lg font-semibold text-gray-900">{{ app()->getLocale() === 'ar' ? $product->salon->name_ar : ($product->salon->name_en ?? $product->salon->name_ar) }}</p>
            </div>
            ...
            <!-- Stock Quantity -->
            <div>
                <label for="stock_quantity" class="block text-sm font-medium text-gray-700 mb-2">{{ __('admin.available_quantity') }} *</label>
            ...
            <div>
                ...
                @if($product->image)
                    <div class="mb-4">
                        <img src="{{ Storage::url($product->image) }}" alt="{{ app()->getLocale() === 'ar' ? $product->name_ar : ($product->name_en ?? $product->name_ar) }}" class="h-40 w-40 object-cover rounded">
